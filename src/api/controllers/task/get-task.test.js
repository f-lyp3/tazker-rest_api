...
describe("Get task's details", () => {
    ...
    it("Should get a task's details", async (done) => {
        ...
        const { body, statusCode } = await getTask(posted._id);
        const { found } = body;
    ...
    it("Should get an 404 error trying to get no-existing task", async (done) => {
        const { body, statusCode } = await getTask("5e8454326b9a0b352e98a9a4");

        expect(statusCode).toBe(404);
        expect(body.error).toBe("Task does not exists!");
        done();
    })

    it("Should get an 401 error trying to get a task with invalid id", async (done) => {
        const { body, statusCode } = await getTask("5e8454326b9a0b35298a9a4");
